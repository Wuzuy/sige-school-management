...
export default function AtendimentoScreen() {
  ...
  const load = async () => {
    try { const r = await request('/aluno/atendimentos'); setData(r || []); } catch {} finally { setLoading(false); } };

  useState(() => { load(); });

  const handleSubmit = async () => {
    if (!tipo || !dataAtendimento || !hora) { Alert.alert('Erro', 'Preencha todos os campos'); return; }
    setSubmitting(true);
    try {
      await request('/aluno/atendimentos', { method: 'POST', body: JSON.stringify({ tipo, data_atendimento: dataAtendimento, hora }) });
      Alert.alert('Sucesso', 'Atendimento agendado');
      setTipo(''); setDataAtendimento(''); setHora(''); setShowForm(false); load();
  ...
  const proximo = data.filter((a: any) => a.status === 'AGENDADO' || a.status === 'CONFIRMADO')
    .sort((a: any, b: any) => new Date(a.data_atendimento).getTime() - new Date(b.data_atendimento).getTime())[0];
    ...
    <ScrollView style={s.container}>
      {proximo && (
        <View style={s.proximoCard}>
          ...
          <Text style={s.proximoAssunto}>{proximo.tipo}</Text>
          <Text style={s.proximoData}>{proximo.data_atendimento?.slice(0, 10) || '-'} as {proximo.hora?.slice(0, 5) || '-'}</Text>
        </View>
      )}
      ...
      {showForm && (
        <View style={s.form}>
          <TextInput style={s.input} placeholder="Tipo (ex: Orientacao, Suporte)" value={tipo} onChangeText={setTipo} />
          <TextInput style={s.input} placeholder="Data (ex: 2026-06-20)" value={dataAtendimento} onChangeText={setDataAtendimento} />
          <TextInput style={s.input} placeholder="Hora (ex: 14:30)" value={hora} onChangeText={setHora} />
      ...
      {data.length === 0 ? (
        ...
        <View key={i} style={s.card}>
          <Text style={s.tipo}>{a.tipo || '-'}</Text>
          <Text style={s.dataHora}>{a.data_atendimento?.slice(0, 10) || '-'} {a.hora?.slice(0, 5) || ''}</Text>
          <Text style={[s.status, { color: a.status === 'AGENDADO' || a.status === 'CONFIRMADO' ? '#e67e22' : a.status === 'REALIZADO' ? '#27ae60' : '#999' }]}>{a.status || '-'}</Text>
          {a.id_responsavel?.nome_completo && <Text style={s.resp}>Responsavel: {a.id_responsavel.nome_completo}</Text>}
        </View>
      ))}
    </ScrollView>
  );
